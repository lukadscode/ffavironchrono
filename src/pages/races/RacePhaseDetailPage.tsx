...
export default function RacePhaseDetailPage() {
  ...
  // Pour les relais, on réduit encore plus la colonne de gauche car elle n'est pas utilisée
  const leftColumnWidth = hasRelays ? "w-full md:w-56 lg:w-64" : "w-full md:w-1/3";

  return (
    <DndContext onDragStart={onDragStart} onDragEnd={onDragEnd}>
      <div className="p-2 sm:p-3 md:p-4 flex flex-col md:flex-row gap-2 md:gap-3 h-[calc(100vh-2rem)] md:h-[calc(100vh-4rem)] overflow-hidden">
        {/* Colonne gauche */}
        <div className={`${leftColumnWidth} space-y-2 md:space-y-3 min-w-0 flex-shrink-0 overflow-y-auto max-h-full`}>
          {previousPhase && (
            <PhaseResultsPanel
        ...
        {/* Colonne droite */}
        <Card className="flex-1 min-w-0 flex flex-col overflow-hidden h-full">
          <CardHeader className="flex flex-col gap-1.5 md:gap-2 lg:gap-3 flex-shrink-0 pb-1.5 md:pb-2 lg:pb-3">
          ...
          <CardContent className="flex-1 overflow-y-auto min-w-0 p-2 md:p-3 lg:p-4">
            <SortableContext items={races.map((r) => r.id)} strategy={verticalListSortingStrategy}>
              <div className="relative pl-3 md:pl-4 lg:pl-6">
                {/* Axe vertical */}
                ...
                {races.map((race, idx) => {
                  ...
                  return (
                    <div key={race.id} className="mb-1.5 md:mb-2">
                      <TimelineRace
                        race={race}
                        timeLabel={timeLabel}
...
function TimelineRace({
  ...
  return (
    <div ref={setNodeRef} style={style} className={clsx("relative border-2 rounded-lg p-2 md:p-2.5 lg:p-3 space-y-1.5 md:space-y-2 lg:space-y-3 bg-white shadow-sm w-full min-w-0", isDragging ? "opacity-70 ring-2 ring-blue-400 shadow-lg" : "hover:border-gray-300", !validation.isValid && "border-red-300 bg-red-50")}>
      <div className="absolute -left-[7px] md:-left-[9px] lg:-left-[11px] top-2.5 md:top-3 lg:top-4 w-3 h-3 md:w-3.5 md:h-3.5 lg:w-4 lg:h-4 rounded-full bg-blue-500 border-2 md:border-2 lg:border-3 border-white shadow-md" />

      <div className="flex items-center justify-between gap-1.5 md:gap-2 min-w-0">
        <div className="font-semibold text-xs md:text-sm flex items-center gap-1.5 md:gap-2 flex-1 min-w-0">
          {onToggleExpand && (
            <button
              onClick={(e) => {
                e.stopPropagation();
          ...
          <span className="inline-flex items-center text-[10px] md:text-xs px-1.5 md:px-2 py-0.5 md:py-1 rounded-md bg-blue-100 text-blue-700 font-mono font-bold flex-shrink-0">{timeLabel}</span>
